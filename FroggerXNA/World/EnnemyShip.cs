...
namespace Frogger.World
{
    ...
    class EnnemyShip : WorldEntity
    {
        ...
        public EnnemyShip(Game game, GraphicsDeviceManager graphics, Vector2 location)
            : base(game, graphics, "content/evilFighter",true)
        {
        ...
        {
            if (mCurrentState == AIState.GoLeft)
            {
                if (mLocation.X >= 20)
                    mLocation.X -= (float)gameTime.ElapsedGameTime.TotalMilliseconds * SPEED;
                else
                    mCurrentState = AIState.GoRight;

            }
            else if (mCurrentState == AIState.GoRight)
            {
                //if (mLocation.X < Background.BACKGROUND_WIDTH - Size.X)
                    mLocation.X += (float)gameTime.ElapsedGameTime.TotalMilliseconds * SPEED;
              //  else
                    mCurrentState = AIState.GoLeft;
            }
